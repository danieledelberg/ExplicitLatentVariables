...
def add_args(parser):
    ...
    group.add_argument('--zinit', choices=('random', 'pretrained', 'zero', 'permuted', 'randomclusters', 'shifted'), required=True, help='How the Z Values should be initialized: random, pretrained, or zero')
...
def train_batch(model, lattice, y, yt, zmu, zvar, rot, trans, optim, zoptim, beta, beta_control=None, tilt=None, ctf_params=None, yr=None, use_amp=False, scaler=None):
    optim.zero_grad()
    zoptim.zero_grad()
    model.train()
    ...
    if scaler is not None:
        with torch.cuda.amp.autocast():
            # Daniel - Make change here, after yt is always zval, do not need to pass into loss function as well
            z_mu, z_logvar, z, y_recon, y_recon_tilt, mask = run_batch(model, lattice, y, yt, zmu, zvar, rot, tilt, ctf_params, yr)
            loss, gen_loss, kld = loss_function(z_mu, z_logvar, y, yt, y_recon, mask, beta, y_recon_tilt, beta_control)
    else:
        # Daniel - change here as well
        z_mu, z_logvar, z, y_recon, y_recon_tilt, mask = run_batch(model, lattice, y, yt, zmu, zvar, rot, tilt, ctf_params, yr)
        loss, gen_loss, kld = loss_function(z_mu, z_logvar, y, yt, y_recon, mask, beta, y_recon_tilt, beta_control)
    if use_amp:
...
# Daniel - edit here to include zval
def run_batch(model, lattice, y, yt, zmu, zvar, rot, tilt=None, ctf_params=None, yr=None):
    ...
    ##############################################
    # Daniel - we skip the encoder entirely, going directly to the given z values
...
def main(args):
    ...
    ###########################################
    # Daniel - Adding in Z values initialized, can be 'random', 'pretrained', 'zero'
    print('Initializing Z Values...')
    # Using pre-existing values
    inittype = args.zinit
    print('Initialization Type: ', inittype)
    
    if inittype == 'pretrained':
        print('Using pre-trained Z values.')
        with open('z.trained.pkl', 'rb') as z_file:
            z_mean_input = pickle.load(z_file)
            z_logvar_input = pickle.load(z_file)
            
    if inittype == 'shifted':
        print('Using pre-trained SHIFTED Z values.')
        shifted_z = np.load('z.shifted.npy')
        z_mean_input = shifted_z[0,...]
        z_logvar_input = shifted_z[1,...]
    
    if inittype == 'random':
        print('Using random Z values.')
        # Can change the logvar to be much larger as a test
        # Uses mean and variance from trained values distribution
        z_mean_input = np.random.normal(loc=-0.05, scale=1.494, size=(len(data), args.zdim))
        if args.randomvar is None:
            z_logvar_input = np.random.normal(loc=-5.47, scale=0.715, size=(len(data), args.zdim))
        elif args.randomvar is not None:
            z_logvar_input = np.random.normal(loc=args.randomvar, scale=0.715, size=(len(data), args.zdim))
            #print(args.randomvar)
        #print('Shape: ', z_mean_input.shape)
        
    if inittype == 'zero':
        print('Using zero initialization.')
        z_mean_input = np.zeros((len(data), args.zdim))
        z_logvar_input = np.ones((len(data), args.zdim)) * (-8)
        print('Mean: ', np.mean(z_mean_input))
        print('Logvar: ', np.mean(z_logvar_input))
    
    if inittype == 'randomclusters':
        print('Using pre-clustered, permuted Z values.')
        with open('z.randomclusters.pkl', 'rb') as z_file:
            z_mean_input, z_logvar_input = pickle.load(z_file)
            
    if inittype == 'permuted':
        print('Using pre-trained, permuted Z values.')
        with open('z.trained.pkl', 'rb') as z_file:
            z_mean_input = pickle.load(z_file)
            z_logvar_input = pickle.load(z_file)
        z_mean_input = np.random.RandomState(seed=42).permutation(z_mean_input)
        z_logvar_input = np.random.RandomState(seed=42).permutation(z_logvar_input)
            
    # If we are loading previous data, use previous z values
    if args.loadz:
        print('Using loaded Z values')
        assert args.load is not None
        with open(args.loadz, 'rb') as z_file:
            z_mean_input, z_logvar_input = pickle.load(z_file)
            print('Imported z values from ', args.loadz)
    ...
    if zlearningrate == 0:
        ...
        print('Optimizer Type: SparseAdam')
        z_optimizer = torch.optim.SparseAdam(list(ztracker.parameters()), lr=zlearningrate)
    
    print('Z Optimizer Initialized.')
    print('Using random noise to train z')
    print('Using KLD')
    ###########################################
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        batch_it = 0 
        for minibatch in data_generator:
            ind = minibatch[-1].to(device) # allows to track indices
            y = minibatch[0].to(device)
            ...
            # Daniel - To get the right z value
        ...
        if args.checkpoint and epoch % args.checkpoint == 0:
            ...
            # Save the Z values, not the same as the eval ones, ignore those
            out_z = '{}/z_vals.{}.pkl'.format(args.outdir, epoch)
            ztracker.save(out_z)
