...
class NoisyMNIST(Dataset):
    ...
    def __getitem__(self, index):
        ...
        noisy = noisy + torch.normal(mean=(torch.ones(noisy.shape)/2), std=0.5)
...
def loss_function(z_mu, z_logvar, y, y_recon, beta):
    ...
    gen_loss = F.mse_loss(y_recon, y.view(B,-1), reduction='sum')
...
def main():
    ...
    test_dataset = NoisyMNIST() #Use test as train
    ...
    print('Generating Noisy Z Values...')
    colors = [tup[1] for tup in test_dataset]
    ...
    for i in range(5):
        ...
        plt.imshow(model((test_dataset[i][0]).to(device))[0].reshape((28,28)).cpu().detach().numpy())
        plt.show()
