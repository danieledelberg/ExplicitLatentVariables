...
def main():
    ...
    out_dir = 'noencoder_pretrained_notrain'
    num_epochs = 100
    beta = 1
    batch_size = 128
    learning_rate = 0.01
    ...
    z_mean_input, z_logvar_input = pickle.load(open('noencoder_zeros/z_vals.90.pkl', 'rb'))
    
    #print('Using normal initialization for latent space')
    #z_mean_input = np.random.normal(loc=0.0, scale=5.0, size=(len(train_dataset), zdim))
