...
class ShiftedMNIST(Dataset):
    ...
    def __getitem__(self, index):
        ...
        shifted_data = torch.roll(shifted_data, shifts=self.shift, dims=1)
        return shifted_data.flatten(), label, index
...
def main():
    ...
    test_dataset1 = ShiftedMNIST(1) # This is the shifted test
    test_dataset5 = ShiftedMNIST(5)
    train_loader = DataLoader(train_dataset, batch_size=batch_size, shuffle=True)
    test_loader1 = DataLoader(test_dataset1, batch_size=60000, shuffle=False)
    test_loader5 = DataLoader(test_dataset5, batch_size=60000, shuffle=False)
    ...
    print('Generating Shifted Z Values (Shift 1)...')
    colors = [tup[1] for tup in test_dataset1]
    # This is the shifted set
    for i in test_loader1:
    ...
    for i in range(20):
        ...
        plt.imshow(test_dataset1[i][0].reshape((28,28)))
        plt.show()

        plt.title('Generated Image')
        plt.imshow(model((test_dataset1[i][0]).to(device))[0].reshape((28,28)).cpu().detach().numpy())
        plt.show()
        
        
    print('Generating Shifted Z Values (Shift 5)...')
    colors = [tup[1] for tup in test_dataset5]
    # This is the shifted set
    for i in test_loader5:
        zmu, zlogvar = model.encoder(i[0].to(device))
    ...
    for i in range(20):
        plt.title('Original Image')
        plt.imshow(test_dataset5[i][0].reshape((28,28)))
        plt.show()

        plt.title('Generated Image')
        plt.imshow(model((test_dataset5[i][0]).to(device))[0].reshape((28,28)).cpu().detach().numpy())
        plt.show()
